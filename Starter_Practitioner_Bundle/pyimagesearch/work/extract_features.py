...
# 抓取我们将描述的图像列表，然后随机洗牌他们，以方便训练和测试分割通过数组切片
...
model = VGG16(weights="imagenet", include_top=False)  # 表示去掉VGG16网络最后的FC层

# 初始化HDF5数据集写入器，然后存储类标签在数据集中
dataset = HDF5DatasetWriter.HDF5DatasetWriter((len(imagePaths), 512 * 7 * 7), args["output"],
